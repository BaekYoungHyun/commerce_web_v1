...
const filteredUsers = computed(() => {
  ...
  return adminUsers.value.filter((user) =>
    [user.seq, user.userId, user.name, user.phone, user.status].some((value) =>
...
const filteredProfiles = computed(() => {
  ...
  return businessProfiles.value.filter((profile) =>
    [
      profile.seq,
...
function hydrate(item: BusinessProfile | WholesaleStore | RetailStore) {
  if ('approvalStatus' in item) {
    ...
    Object.assign(form, {
      businessProfileSeq: String(item.businessProfileSeq),
      storeName: item.storeName,
      marketName: item.marketName ?? '',
      floorRoom: item.floorRoom ?? '',
      status: item.status,
    })
...
async function initialize() {
  ...
  try {
    ...
    if (!item) {
      localError.value = '수정할 정보를 목록에서 찾을 수 없습니다.'
      return
    }
...
function payload(): AdminBusinessRequest {
  if (resource.value === 'business-profiles')
    return {
      userSeq: Number(form.userSeq),
      businessNumber: form.businessNumber.trim(),
      companyName: form.companyName.trim(),
      representativeName: form.representativeName.trim(),
      approvalStatus: form.approvalStatus.trim(),
      approvedAt: form.approvedAt || null,
    }
  if (resource.value === 'wholesale-stores')
    return {
      businessProfileSeq: Number(form.businessProfileSeq),
...
async function submit() {
  ...
  if (
    ...
    localError.value = '필수 입력값과 ID를 확인해 주세요.'
    return
  }
  try {
    await store.save(resource.value, body, editSeq.value)
    await router.push(config.value.list)
  } catch (cause) {
    if (cause instanceof ApiError && cause.status === 401)
      await router.push({ path: '/login', query: { redirect: route.fullPath } })
  }
...
watch(
  ...
  (status, previousStatus) => {
    if (status === 'APPROVED' && previousStatus !== 'APPROVED' && !form.approvedAt)
      form.approvedAt = nowLocalDateTime()
...
<template>
  <main class="admin-content admin-business-form-content">
    <nav class="admin-breadcrumbs">
      <RouterLink :to="config.list">{{ config.title }} 관리</RouterLink><span>›</span
      ><strong>{{ isEdit ? '수정' : '등록' }}</strong>
    </nav>
    <div class="admin-page-heading admin-form-page-heading">
      <div>
        <p>SERVICE ADMIN</p>
        <h1>{{ config.title }} {{ isEdit ? '수정' : '등록' }}</h1>
        <span>{{
          resource === 'business-profiles'
            ? '대표 사용자를 선택하고 사업자 승인 상태를 관리합니다.'
            : '매장 운영 상태를 선택해 관리합니다.'
        }}</span>
      </div>
    </div>
    <form class="admin-form-section admin-business-form" @submit.prevent="submit">
      <div class="admin-form-section-heading">
        <div>
          <strong>기본 정보</strong
          ><span>별표 항목은 필수입니다. nullable 항목은 비워두면 null로 전송합니다.</span>
        </div>
      </div>
      ...
      <div v-else class="admin-form-grid">
        <template v-if="resource === 'business-profiles'">
          <label class="admin-user-picker-field"
            ><span>대표 사용자 *</span
            ><button type="button" class="admin-user-picker-button" @click="openUserPicker">
              <strong v-if="form.userSeq">SEQ {{ form.userSeq }}</strong
              ><span v-if="selectedUser.userName"
                >{{ selectedUser.userName
                }}<small v-if="selectedUser.userId">({{ selectedUser.userId }})</small></span
              ><span v-else>{{ form.userSeq ? '선택된 사용자' : '대표 사용자 선택' }}</span
              ><b>검색</b></button
            ><small>{{ fieldErrors.userSeq }}</small></label
          >
          <label
            ><span>사업자번호 *</span><input v-model="form.businessNumber" required /><small>{{
              fieldErrors.businessNumber
            }}</small></label
          >
          <label
            ><span>사업자명 *</span><input v-model="form.companyName" required /><small>{{
              fieldErrors.companyName
            }}</small></label
          >
          <label
            ><span>대표자명 *</span><input v-model="form.representativeName" required /><small>{{
              fieldErrors.representativeName
            }}</small></label
          >
          <label
            ><span>승인 상태 *</span
            ><select v-model="form.approvalStatus" required>
              <option v-for="status in approvalStatuses" :key="status.value" :value="status.value">
                {{ status.label }} ({{ status.value }})
              </option></select
            ><small>{{ fieldErrors.approvalStatus }}</small></label
          >
          <label
            ><span>승인 일시</span
            ><input
              :value="
                form.approvedAt ? formatDateTime(form.approvedAt) : '승인 상태 선택 시 자동 등록'
              "
              readonly
            /><small>{{ fieldErrors.approvedAt }}</small></label
          >
        </template>
        <template v-else>
          <label class="admin-user-picker-field"
            ><span>사업자 프로필 *</span
            ><button type="button" class="admin-user-picker-button" @click="openProfilePicker">
              <strong v-if="form.businessProfileSeq">SEQ {{ form.businessProfileSeq }}</strong
              ><span v-if="selectedProfile.companyName"
                >{{ selectedProfile.companyName
                }}<small v-if="selectedProfile.businessNumber"
                  >({{ selectedProfile.businessNumber }})</small
                ></span
              ><span v-else>{{
                form.businessProfileSeq ? '선택된 사업자 프로필' : '사업자 프로필 선택'
              }}</span
              ><b>검색</b></button
            ><small>{{ fieldErrors.businessProfileSeq }}</small></label
          >
          <label
            ><span>사업장명 *</span><input v-model="form.storeName" required /><small>{{
              fieldErrors.storeName
            }}</small></label
          >
          <label v-if="resource === 'wholesale-stores'"
            ><span>시장명</span><input v-model="form.marketName" /><small>{{
              fieldErrors.marketName
            }}</small></label
          >
          <label v-if="resource === 'wholesale-stores'"
            ><span>층/호수</span><input v-model="form.floorRoom" /><small>{{
              fieldErrors.floorRoom
            }}</small></label
          >
          <label v-if="resource === 'retail-stores'"
            ><span>판매 채널</span><input v-model="form.salesChannel" /><small>{{
          ...
          <label
            ...
            ><select v-model="form.status" required>
              <option
                v-if="form.status && !storeStatuses.some((item) => item.value === form.status)"
                :value="form.status"
              >
                {{ form.status }} (기존 값)
              </option>
              <option v-for="status in storeStatuses" :key="status.value" :value="status.value">
                {{ status.label }} ({{ status.value }})
              </option></select
            ><small>{{ fieldErrors.status }}</small></label
          >
        </template>
      </div>
      ...
      <div class="admin-form-footer">
        <RouterLink :to="config.list">취소</RouterLink
    ...
    <div
      ...
      role="presentation"
      @click.self="userPickerOpen = false"
    >
      <section
      ...
      >
        <div class="admin-modal-heading">
          <div>
            <p>REPRESENTATIVE USER</p>
            <h2 id="user-picker-title">대표 사용자 선택</h2>
          </div>
          <button type="button" aria-label="팝업 닫기" @click="userPickerOpen = false">×</button>
        </div>
        <p class="admin-picker-guide">사용자 목록에서 대표자로 연결할 계정을 선택하세요.</p>
        <label class="admin-picker-search"
          ><span>사용자 검색</span
          ><input v-model="userKeyword" placeholder="SEQ, 아이디, 이름, 연락처, 상태"
        /></label>
        <p v-if="usersError" class="admin-list-error" role="alert">
          {{ usersError }} <button type="button" @click="usersStore.fetchUsers()">다시 시도</button>
        </p>
        <div class="admin-user-picker-list">
          ...
          <button
            ...
            :key="user.seq"
            type="button"
            :class="{ selected: Number(form.userSeq) === user.seq }"
            @click="chooseUser(user)"
          >
            <span
              ><strong>{{ user.name }}</strong
              ><small>{{ user.userId }} · {{ user.phone }}</small></span
            ><span
              ><i>{{ user.status }}</i
              ><b>SEQ {{ user.seq }}</b></span
            >
          </button>
    ...
    <div
      ...
      class="admin-modal-backdrop"
      role="presentation"
      @click.self="profilePickerOpen = false"
    >
      <section
        class="admin-product-modal admin-user-picker-modal"
        role="dialog"
      ...
      >
        <div class="admin-modal-heading">
          <div>
            ...
            <h2 id="profile-picker-title">사업자 프로필 선택</h2>
          </div>
          <button type="button" aria-label="팝업 닫기" @click="profilePickerOpen = false">×</button>
        </div>
        <p class="admin-picker-guide">매장과 연결할 사업자 프로필을 선택하세요.</p>
        <label class="admin-picker-search"
          ><span>사업자 프로필 검색</span
          ><input
            v-model="profileKeyword"
            placeholder="SEQ, 사업자번호, 사업자명, 대표자, 사용자, 승인 상태"
        /></label>
        <p v-if="error" class="admin-list-error" role="alert">
          {{ error }} <button type="button" @click="openProfilePicker">다시 시도</button>
        </p>
        <div class="admin-user-picker-list admin-profile-picker-list">
          ...
          <button
            ...
            :key="profile.seq"
            type="button"
            :class="{ selected: Number(form.businessProfileSeq) === profile.seq }"
            @click="chooseProfile(profile)"
          >
            <span
              ><strong>{{ profile.companyName }}</strong
              ><small>{{ profile.businessNumber }} · 대표 {{ profile.representativeName }}</small
              ><small
                >{{ profile.userName
                }}<template v-if="profile.userId"> ({{ profile.userId }})</template></small
              ></span
            ><span
              ><i>{{ profile.approvalStatus }}</i
              ><b>SEQ {{ profile.seq }}</b></span
            >
          </button>
          <p v-if="!profileLoading && filteredProfiles.length === 0">
            조건에 맞는 사업자 프로필이 없습니다.
          </p>
        </div>
      </section>
    </div>
