...
describe('orderApi', () => {
  ...
  it('선택한 CartItem.seq와 수령 정보를 주문 생성 API에 전송한다', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      expect.stringContaining('/api/v1/orders/from-cart'),
  ...
  it('셀러 주문 취소를 body 없이 요청한다', async () => {
    const canceled = {
      seq: 9,
      orderNo: 'O-9',
      status: 'CANCELED',
    ...
    const fetchMock = vi.fn<typeof fetch>().mockResolvedValue(response(canceled))
    vi.stubGlobal('fetch', fetchMock)

    await expect(orderApi.cancel('token', 9)).resolves.toEqual(canceled)
    expect(fetchMock).toHaveBeenCalledWith(
      expect.stringContaining('/api/v1/seller/orders/9/cancel'),
