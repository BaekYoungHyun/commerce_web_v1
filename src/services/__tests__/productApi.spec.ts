...
describe('productApi', () => {
  ...
  it('상품 조회 로그를 요청 body 없이 토큰 사용자로 등록한다', async () => {
    const view = { seq: 7, userSeq: 3, productSeq: 1, viewedAt: '2026-07-31T10:00:00+09:00' }
    const fetchMock = vi.fn<typeof fetch>().mockResolvedValue(jsonResponse(view, 201))
    vi.stubGlobal('fetch', fetchMock)

    await expect(productApi.createView('token', 1)).resolves.toEqual(view)
    expect(fetchMock).toHaveBeenCalledWith(
  ...
  it('상품 수정은 PUT과 전체 필수 필드를 전송한다', async () => {
    ...
    const fetchMock = vi
      .fn<typeof fetch>()
      .mockResolvedValue(jsonResponse({ seq: 1, ...payload, createdAt: '', updatedAt: '' }))
    vi.stubGlobal('fetch', fetchMock)
